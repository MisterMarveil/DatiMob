...
public class LinphoneMiniManager implements LinphoneCoreListener {
    ...
    public void newOutgoingCall(String to, String displayName) {
        ...
        if(mLinphoneCore.isNetworkReachable()) {
			...
			//callbackContext.error("Network unreachable");
        }
	...
	public void callState(LinphoneCore lc, LinphoneCall call, State cstate,
			...
			{
				mCallbackContext.sendPluginResult(new PluginResult(PluginResult.Status.OK, "Connected message:"+message));
			}
			else if(cstate == State.IncomingReceived)
			{
                mCallbackContext.sendPluginResult(new PluginResult(PluginResult.Status.OK,"Incoming message:"+message));
			}
			else if(cstate == State.CallEnd)
			{
				mCallbackContext.sendPluginResult(new PluginResult(PluginResult.Status.OK,"End message:"+message));
			}
			else if(cstate == State.Error)
			{
				mCallbackContext.sendPluginResult(new PluginResult(PluginResult.Status.OK,"Error message:"+message));
			}
			else
			{
				mCallbackContext.sendPluginResult(new PluginResult(PluginResult.Status.OK,"callState:"+cstate+" message: "+message));
			}
